...
# bond_existごとの頻度分布 (強い)
# +,-はこれで大別される？
train %>% 
  # head(100000) %>% 
  ggplot(aes(x=factor(bond_exist),y=scalar_coupling_constant)) +
  geom_boxplot() +
  theme_minimal()

# 電気陰性度の差とtarget (そもそも原子違うから, あんまり関係ない)
train %>% 
  filter(bond_exist == 1) %>% 
  ggplot(aes(x=electro_nega_diff, y = scalar_coupling_constant)) +
  geom_point() +
  theme_minimal()

# bond_exist == 0, via_number = 1でも負にはならない
train %>% 
  filter(bond_exist == 0) %>%
  # head(1000000) %>% 
